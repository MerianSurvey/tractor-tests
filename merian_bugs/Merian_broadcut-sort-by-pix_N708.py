...
for obj in obj_cat: # one galaxy only
    ...
    pkl_path = f'/data/users/dcblanco/Merian/{folder}/{obj_id}/tractor/cosmos_{obj_id}_sep_tractor_N708.pkl'
    ...
    if os.path.exists(pkl_path):
        ...
        try: 
            # for Merian channel
            trac_pos1 = np.array([model_dict['N'].catalog.subs[count].pos[0] for count, source in enumerate(model_dict['N'].catalog)])
            trac_pos2 = np.array([model_dict['N'].catalog.subs[count].pos[1] for count, source in enumerate(model_dict['N'].catalog)])
            trac_pos = np.stack((trac_pos1, trac_pos2), axis = -1)
            target_source = np.argmin([np.sqrt((merian_center[0]-pos[0])**2 + (merian_center[1]-pos[1])**2) for ct, pos in enumerate(trac_pos)]) # I am fairly confident this works.. 
            target_model_dict['N'] = model_dict['N'].catalog.subs[target_source]
        except: 
            pass  
        
        os.chdir(f'/data/users/dcblanco/Merian/{folder}/{obj_id}/tractor/')
        with open(f'cosmos_{obj_id}_sep_tractor_N708_target.pkl', 'wb') as f: 
