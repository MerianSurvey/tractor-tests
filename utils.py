...
def tractor_iteration(obj_cat, w, img_data, invvar, psf_obj, pixel_scale,
    ...
    # compute inverse variance (error) of the flux 
    flux_ivars = _compute_invvars(all_derivs)[0]
    print('Target source flux inverse variance:', flux_ivars) 
        
    return sources, trac_obj, fig, flux_ivars
...
def tractor_hsc_cmodel(obj_name, coord, s_ang, filt, channels, data, hsc_dr, 
    ...
    while True:
        try:
            if kfold == 1:
                break
            sources, trac_obj, fig, flux_ivars = tractor_iteration(
                cutout_clean,
                data.wcs,
...
def tractor_hsc_sep(obj, filt, channels, data, brick_file='../survey-bricks-dr9.fits.gz', 
    ...
    return trac_obj, flux_ivars


### function from legacysurvey
