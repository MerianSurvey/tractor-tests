...
import kuaizi.download as download
import kuaizi
...
def rubber_tractor_iteration(obj_cat, w, img_data, invvar, psf_obj, pixel_scale,
                      shape_method='manual', freeze_pos=True, freeze_shape=True, freeze_sersic=True, 
    ...
    freeze_shape (bool): whether freezing the shape of objects during fitting. 
    freeze_sersic (bool): whether freezing the sersic index of objects during fitting.
    Returns:
    -----------
...
MODIFIED FUNCTION: 
'''
def rubber_tractor_hsc_sep(obj, filt, channels, data, brick_file='../survey-bricks-dr9.fits.gz', freeze_pos=True, freeze_shape=True, freeze_sersic=True, verbose=False):
    '''
    Run `the tractor` on HSC images, for Merian survey.
